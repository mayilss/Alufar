...
export const Product = () => {
    ...
    useEffect(() => {
        const fetchSimilarProducts = async () => {
            try {
                const res = await axios(
                    process.env.REACT_APP_API_URL + `/api/products/${slug}?lang=az&item=4`
                );
        ...
        const fetchProduct = async () => {
            try {
                const res = await axios(
                    process.env.REACT_APP_API_URL + `/api/products/product-1?lang=az`
                );
                ...
                <div className={styles.similar}>
                    ...
                    <div className="row">
                        {similarProducts.map(item => {
                            return (
                                <div onClick={()=>{handleItem(item.slug)}} key={item.id} className="col-md-3 col-6">
                                    <ProjectCard
